...
pub async fn create_profile(
    ...
    for (file_name, _original_content_type, original_bytes) in photos {
        ...
        const MAX_SIZE: usize = 400_000; // KB
...
pub async fn get_photos_of_profile(
    ...
    loop {
        let result = sqlx::query!(r#"SELECT id FROM photos WHERE profile_id = $1"#, profile_id)
            .fetch_all(&data.db)
            .await;
...
pub async fn delete_profile(
    State(data): State<Arc<AppState>>,
    AuthenticatedViewer { is_admin, .. }: AuthenticatedViewer,
    Path(id): Path<Uuid>,
) -> Result<impl IntoResponse, (StatusCode, Json<serde_json::Value>)> {
    if !is_admin {
        return Err((
